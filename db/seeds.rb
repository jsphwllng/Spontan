...
event_6 = Event.new(category: "dancing", description: "We are a small group of enthusiastic salza dancers. Everyone is welcome! No prior knowledge required :)", location: "Volkspark Wedding", date: (DateTime.now + (score.sample + 21000).hours), user: user_2, title: "Salsa Dancing")
event_7 = Event.new(category: "studying", description: "The semester is almost over and exams are just around the corner. I give free tips & tricks how to step up your learning process when it comes to studying for exams.", location: "Staatsbibliothek Berlin", date: (DateTime.now + (score.sample + 21000).hours), user: user_3, title: "Studying tips")
event_8 = Event.new(category: "party", description: "Lets end the day with some pumping beats and some nice & cold beers, 18+ !", location: "Café Hasenschänke", date: (DateTime.now + (score.sample + 21000).hours), user: user_4, title: "Beers & Beats")
...
event_13 = Event.new(category: "party", description: "Let's plat Doppelkopf at Laidak, preferably without 9s though!", location: "Boddinstr. 8", date: (DateTime.now + (score.sample + 2222222).hours), user: user_5, title: "Doppelkopf!")

event_14 = Event.new(category: "party", description: "Betrayal on the House of the Hill is probably the best game ever invented. If you like strategy/group games this is perfect for you! Let's play!", location: "Hermannplatz", date: (DateTime.now + (score.sample + 2222222).hours), user: user_3, title: "Let's play Betrayal!")

event_15 = Event.new(category: "misc", description: "I love how to knit and am looking for likeminded people or those who would like to learn how to knit. Winter is comimg and I think we could all use some warm woolen socks soon :) ", location: "Checkpoint Charlie", date: (DateTime.now + (score.sample + 2222222).hours), user: user_5, title: "Let's knit socks")

event_16 = Event.new(category: "misc", description: "I'd like to learn how to read music and thought maybe you'd like to as well? Or maybe you are an experienced musician looking to pass on your wisdom - either would be swell! I am not a complete beginnder but would definitely benefit from an exchange of experiences ", location: "Motzstr. 30", date: (DateTime.now + (score.sample + 2222222).hours), user: user_1, title: "Reading music")

event_17 = Event.new(category: "sports", description: "Corona made me fat and I'd really like to walk the extra weight off. But it's always easier to stay motivated if I do thinks in a group so here is me hoping you feel the same way.", location: "Bouchéstr. 3", date: (DateTime.now + (score.sample + 2222222).hours), user: user_7, title: "Nordic Walking")
...
event_21 = Event.new(category: "social", description: "I love getting manicures and would love to make a little event out of it, so I thought we would get a drink at a nearby bar and go for manicures after a few cocktails :)", location: "Rosenthaler Platz", date: (DateTime.now + (score.sample + 222222).hours), user: user_4, title: "Manicures 'n drinks")

event_22 = Event.new(category: "social", description: "We all need more love and affection in our lives so how about a biiiig group hugging session so share and spread the love! We will obviously wear masks and not get unreasonably touchy.", location: "Mauerpark", date: (DateTime.now + (score.sample + 222222).hours), user: user_9, title: "Group hugs!")

event_23 = Event.new(category: "food", description: "I loooooove baking and would like to try a new recipe with you guys! I thought about making a nougat ganache torte with a pistachio buttercream finish. Obviously we will have some coffee and and eat the whole thing afterwards!", location: "Hausvogteiplatz", date: (DateTime.now + (score.sample + 222222).hours), user: user_10, title: "Baking in Berlin")
